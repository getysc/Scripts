<#
.SYNOPSIS
Generates a report of subnets.

.DESCRIPTION
Generates a report of subnets grouped by VPC.
...
Process {
    ...
    foreach ($Vpc in $VpcList) {
        ...
        Write-Information -MessageData "- $($Vpc.VpcId)" -InformationAction Continue
        Write-Information -MessageData ("-" * 80) -InformationAction Continue

        # Get just subnets for this VPC.
        $SubnetsInVpc = $SubnetList | `
            Where-Object -Property VpcId -eq $Vpc.VpcId | `
            Sort-Object -Property AvailabilityZone, CidrBlock

        if ($SubnetsInVpc) {
            foreach ($Subnet in $SubnetsInVpc) {
                ...
                # Output this subnet.
                Write-Information -MessageData ("  " + $Subnet.AvailabilityZone + " " + `
                    $Subnet.CidrBlock.PadRight(18) + " " + `
                    $PublicOrPrivate + " " + `
                    $Subnet.SubnetId) -InformationAction Continue
            }
        } else {
            # No subnets in this VPC.
            Write-Information -MessageData "  No Subnets" -InformationAction Continue
        }

        # Add a space in between VPCs.
